...
# Loading basic information into memory
schemes: List[Dict[str, Any]] = [
    {
        "id": "tractor-loan",
        "name": "Tractor Loan Scheme",
...
- Instruction: "Extract the name value" | User: "My name is John" | Response: "John"
- Instruction: "Extract the age value" | User: "My name is John" | Response: "Not Found"
- Instruction: "Extract the address value" | User: "Chennai" | Response: "Chennai"
- Instruction: "Extract the address value" | User: "chennai" | Response: "chennai"
- Instruction: "Extract the earning value" | User: "2 lakhs" | Response: "200000"
- Instruction: "Extract the community value" | User: "2 lakhs" | Response: "Not Found"
- Instruction: "Extract the community value" | User: "OBC" | Response: "OBC"
- Instruction: "Extract the community value" | User: "I belong to sc" | Response: "sc"
- Intruction: "Extract the situation value" | User: "To buy a two wheeler, you need a loan." | Response: "To buy a two wheeler, you need a loan."
- Intruction: "Extract the situation value" | User: "You need a loan to buy a two wheeler" | Response: "TYou need a loan to buy a two wheeler"
"""
...
try:
    ...
    print("WHISPER MODEL LOADED PROPERLY")
except Exception as e:
    print(e)
    print(f"WHISPER MODEL DID NOT LOAD AT ALL")
...
def is_valid_situation(situation_text: str, user_data: Dict[str, Any] = None) -> bool:
    ...
    if user_data is not None:
        ...
        if len(matched) > 0:
            ...
            print(f"[SITUATION VALIDATION] Situation does not match any schemes, falling back to keyword check")
    
    # Fallback: check if it contains relevant keywords
    text_lower = situation_text.lower().strip()
    keywords = get_all_situation_keywords()
...
@app.route("/start_agent", methods=["POST"])
def start_agent():
    print("CALLING START AGENT FUNCTION")
    payload = request.get_json() or {}
    # Stateless start: return the first question and audio for a fresh interaction.
    language = payload.get("language", "ta")
    next_index = 0
...
@app.route("/agent_step", methods=["POST"])
def agent_step():
    print("CALLING AGENT STEP FUNCTION")
    ...
    try:
        ...
        while i < len(FIELDS):
            ...
            print(instruction)
            extracted = "Not Found"
            print("CAME TILL INSTRUCTIONS")
            if transcript_en:
                print("CAME INSIDE TRANSCRIPT EN")
                try:
                    extracted = asyncio.run(extract_field_with_llm(instruction, transcript_en))
                    print(f"[LLM EXTRACT] field={key}, extracted={extracted}")
                    # Clean up the extracted value - remove any labels, prefixes, or extra text
            ...
            print("CAME OUT OF TRANSCRIPT EN")
            # Validate extracted value - must not be empty, "Not Found", or contain field labels
            is_valid = (
                extracted != "Not Found" 
                and extracted.strip() != ""
            ...
            print("IS VALID FOR NAME: ", is_valid)
        ...
        field_name_print = FIELDS[next_index]["key"] if next_index < len(FIELDS) else "done"
        print("THE NEXT INDEX: ", next_index, "field name: ", field_name_print)
...
- Instruction: "Extract the community value" | User: "OBC" | Response: "OBC"
- Instruction: "Extract the community value" | User: "I belong to sc" | Response: "sc"
- Instruction: "Extract the situation value" | User: "I need a loan to buy a tractor" | Response: "I need a loan to buy a tractor"
"""
...
try:
    ...
    print(e)
    print("WHISPER MODEL DID NOT LOAD AT ALL")
...
def transcribe_file_translate_to_english_from_path(path: str) -> str:
    ...
    try:
        ...
        if not os.path.exists(path) or os.path.getsize(path) == 0:
            print(f"[TRANSCRIBE] file missing or empty: {path}")
            return ""
        segments, info = whisper_model.transcribe(path, task="translate", language="ta")
        text = "".join([seg.text for seg in segments])
        print("TRANSCRIPTION COMPLETE:", text)
        return text.strip()
    except Exception as e:
...
# ---------- FIX 1: Synchronous LLM — no asyncio.run(), no event loop issues ----------

def extract_field_with_llm_sync(instruction: str, user_text: str) -> str:
    """
    Calls the LLM synchronously using llm.invoke().
    The original asyncio.run() was causing 'Event loop is closed' errors
    because Flask runs in a threaded context where the event loop created
    by asyncio.run() on request 1 gets closed before request 2 arrives.
    llm.invoke() is the correct synchronous path — no event loop needed.
    """
    if llm is None:
        return "Not Found"
    try:
...
# ---------- FIX 2: simple_extract — bypass LLM for basic fields ----------

def simple_extract(key: str, transcript: str) -> str:
    """
    For name, age, address, earning — the Whisper transcript IS the answer.
    No LLM call needed. This also avoids OOM errors on low-RAM machines.
    """
    transcript = transcript.strip()
    if not transcript:
        return "Not Found"

    SIMPLE_FIELDS = {"name", "age", "address", "earning"}
    ...
    # name and address: the transcript itself is the answer
    if len(transcript) >= 2:
        return transcript
...
def print_jsonrpc(collected: Dict[str, Any], user_id: str = None) -> None:
    """Prints a JSON-RPC message to the terminal after every successful field extraction."""
    try:
        earning_raw = collected.get("earning", "")
        try:
            earning_int = int(str(earning_raw).replace(",", "")) if earning_raw else None
        except (ValueError, TypeError):
            earning_int = earning_raw if earning_raw else None
        ...
        rpc_message = {
            ...
            "params": {
                ...
                "user_metadata": {
                    ...
                    "age": collected.get("age") or None,
                    "community": collected.get("community") or None,
                    "yearly_earning_proxy": earning_int,
                },
                "situational_context": collected.get("situation") or None,
                "source": "voice_agent",
                "timestamp": timestamp,
            }
        }
        ...
        print("\n" + "=" * 60)
        print("JSON-RPC MESSAGE (current state):")
        print(json.dumps(rpc_message, indent=2, ensure_ascii=False))
        print("=" * 60 + "\n")
    except Exception as e:
        print(f"[JSON-RPC PRINT ERROR] {e}")
...
def agent_step():
    ...
    try:
        ...
        while i < len(FIELDS):
            ...
            if transcript_en:
                # FIX 2: simple_extract first — no LLM needed for basic fields
                simple_result = simple_extract(key, transcript_en)
                if simple_result != "Not Found":
                    extracted = simple_result
                    print(f"[SIMPLE EXTRACT] field={key}, extracted={extracted}")

                elif key == "situation":
                    extracted = transcript_en.strip() if transcript_en else "Not Found"
                    print(f"[SITUATION] Using raw transcript as situation: {extracted}")
                    if not extracted:
                        extracted = "Not Found"

                else:
                    # FIX 1: synchronous invoke() for any other future fields
                    instruction = f"Extract the {key} value from the user's reply. If you cannot find it, return exactly: Not Found"
                    print(instruction)
                    extracted = extract_field_with_llm_sync(instruction, transcript_en)
                    print(f"[LLM EXTRACT] field={key}, extracted={extracted}")
                    extracted = (extracted or "").strip()
                    if "not found" in extracted.lower() or len(extracted) == 0:
                        extracted = "Not Found"
                    if f"{key}" in extracted.lower() and ":" in extracted:
                        ...
                        if len(parts) > 1:
                            extracted = parts[1].strip()
                    if any(word in extracted.lower() for word in ["value", "extracted", "found"]):
                        if key.lower() not in extracted.lower():
                            extracted = "Not Found"
                    extracted = extracted.strip() if extracted else "Not Found"
            ...
            is_valid = (
                extracted != "Not Found"
                and extracted.strip() != ""
                and not any(label in extracted.lower() for label in [
                    "value:", "extracted:", "found:", "community value:", "situation value:",
                    "earning value:", "age value:", "name value:", "address value:"
                ])
            )

            print(f"IS VALID FOR {key}: {is_valid}")
            ...
            if key == "situation" and is_valid:
                ...
                if not is_valid_situation(situation_text_to_check, validation_user_data):
                    print(f"[SITUATION VALIDATION] '{extracted}' does not match any schemes, storing anyway and marking done")
                    # Store it anyway so the loop doesn't repeat — schemes just won't match
                    collected[key] = extracted
                    values_extracted[key] = extracted
            ...
            '''if key == "situation" and is_valid:
                ...
                if not is_valid_situation(situation_text_to_check, validation_user_data):
                    print(f"[SITUATION VALIDATION] '{extracted}' does not match any schemes, treating as invalid")
                    is_valid = False
                else:
                    print(f"[SITUATION VALIDATION] '{extracted}' matches at least one scheme, accepting")'''
            ...
            if is_valid:
                ...
                values_extracted[key] = extracted
                print(f"[VALID EXTRACTION] field={key}, value={extracted}")
                # Print JSON-RPC after every successful extraction
                print_jsonrpc(collected)
                i += 1
                break
            else:
        ...
        if not done and next_index < len(FIELDS):
            ...
            agent_text = "அனைத்து விவரங்களும் சேகரிக்கப்பட்டது. நன்றி! விவரங்கள் தயாரிப்பு முகவருக்கு அனுப்பப்பட்டது." if language == "ta" else "All details collected. Thank you! Data sent to the Product Agent."
            field_key = FIELDS[-1]["key"] if FIELDS else ""
            print("\n[FINAL JSON-RPC — All fields collected]")
            print_jsonrpc(collected)
...
def validate_scheme_field_value(field_key: str, value: str) -> bool:
    if not value or value.strip() == "":
        return False
    if field_key in ("farmSize", "land_size"):
        return bool(re.search(r'\d+', value))
    elif field_key in ("cropType", "tractor_model"):
        return len(value) >= 2 and not value.isdigit()
    elif field_key in ("annualIncome", "loanAmount", "loan_amount"):
        return bool(re.search(r'\d+', value))
    return len(value) >= 1
...
def validate_scheme_field():
    ...
    language = payload.get("language", "ta")
    is_valid = validate_scheme_field_value(field_key, value)
    agent_text = "✓" if is_valid else (
        "மீண்டும் முயற்சிக்கவும். தயவுசெய்து சரியான பதிலை வழங்கவும்." if language == "ta"
        else "Please try again. Please provide a valid answer."
    )
    return jsonify({"is_valid": is_valid, "agent_text": agent_text})
...
def submit_and_match():
    ...
    matching_schemes = get_matching_schemes({"age": age, "community": community}, situation_gist)
    if not matching_schemes:
        return jsonify({"status": "error", "message": "No schemes matched.", "json_rpc_sent": json_rpc_payload}), 404
    return jsonify({"status": "success", "message": "Schemes matched.", "schemes": matching_schemes, "json_rpc_sent": json_rpc_payload})
